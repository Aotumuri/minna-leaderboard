...
export function getLeaderBoard(field, sort, limit) {
    return db.prepare(`SELECT ${field.join(', ')} FROM leaderboard ORDER BY ${sort.key} ${sort.type} ${limit ? `LIMIT ${limit}`: ''}`).all();
}
...
export function setDecoy() {
    const setData = db.prepare(`INSERT INTO leaderboard (name, kill, time, score) VALUES (?, ?, ?, ?);`)
    ...
    return getLeaderBoard(["name", "kill", "time", "score"], {"key": "score", "type": "ASC"})
}